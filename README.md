# About-Me
BCA Graduate | MCA Student | Full-Stack MERN Developer 🚀

# Hi there, I'm Asadkhan Jamad 👋

### BCA Graduate | MCA Student | Full-Stack MERN Developer

I am a developer from Ahmedabad, focused on building scalable web apps and automation tools.

- 🎓 **Education**: BCA from GLS University 
- 🎓 **Current**: Pursuing MCA 
- 💻 **Tech Stack**: MongoDB, Express.js, React.js, Node.js (MERN)
- 🐍 **Interests**: Python Automation & Web Scraping

---

### 🛠 Tech Stack & Tools
- **Frontend**: React.js, Redux, HTML5, CSS3, JavaScript (ES6+)
- **Backend**: Node.js, Express.js, REST APIs
- **Database**: MongoDB (Mongoose), SQL
- **Automation**: Python (BeautifulSoup, Requests)

---

### 📊 My GitHub Stats & Top Languages

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&show_icons=true&theme=radical" alt="Asad's Stats" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_GITHUB_USERNAME&layout=compact&theme=radical" alt="Top Languages" />
</p>

---

### 📫 Connect with me:
[LinkedIn](www.linkedin.com/in/asadkhan-jamad-9b4031285) | [Portfolio](YOUR_PORTFOLIO_LINK)
